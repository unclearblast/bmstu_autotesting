...
# 📘 Пособие для самых маленьких: как запустить проект и нагрузочный тест (ПОЛНЫЙ РИДМИ В README.PDF)

Этот файл — пошаговая инструкция для человека, который впервые видит Docker, Gradle и k6.  
Выполняйте шаги строго по порядку, и всё заработает. У меня на машине поднялось только консольно, пробелмыс докером. Вируталка крашится, поэтому плейсхолдер из README.pdf считать настоящим.
---
